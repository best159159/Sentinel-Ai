...
export default function IncidentForm({ selectedLocation, onSelectLocation, onSuccess }: IncidentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!firebaseUser) { setError('Please log in to report an incident'); return; }

    setLoading(true);
    try {
      // 1. Upload image to Firebase Storage (if any)
      let imageUrl: string | null = null;
      if (imageFile) {
        const imageId = crypto.randomUUID();
        const imgRef = storageRef(storage, `incidents/${imageId}/${imageFile.name}`);
        await uploadBytes(imgRef, imageFile);
        imageUrl = await getDownloadURL(imgRef);
      }

      // 2. Call Next.js API route for AI analysis
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl text-sm font-bold transition-all shadow-sm border ${selectedLocation
