...
import { getMyIncidents } from '@/lib/firestore';
...
export default function MyIncidentsPage() {
  ...
  useEffect(() => {
    ...
    const fetchMyIncidentsAsync = async () => {
      if (!firebaseUser) return;
      try {
        const data = await getMyIncidents(firebaseUser.uid);
        
        // Map Firestore data to Incident format
        const formatted = data.map((d: any) => ({
            ...d,
            createdAt: d.createdAt?.toDate?.()?.toISOString() || new Date().toISOString(),
        }));
        
        setIncidents(formatted as unknown as Incident[]);
      } catch (err) {
        console.error(err);
